...
contract Groth16VerifierV5_1Stub {
    ...
    function verifyProof(uint[2] calldata _pA, uint[2][2] calldata _pB, uint[2] calldata _pC, uint[19] calldata _pubSignals) public view returns (bool) {
        // memory-safe annotation lets the IR optimizer pack stack frames
        // without rejecting on "no memoryguard". Required because the
        // 19-public-input verifier hits stack-too-deep under via_ir
        // without it; the legacy 14-input verifier slipped under the
        // threshold. Body is auto-generated by snarkjs and only writes
        // to memory locations bumped past mload(0x40), so memory-safe
        // is correct here.
