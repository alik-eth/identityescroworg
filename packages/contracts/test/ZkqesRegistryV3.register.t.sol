...
contract ZkqesRegistryV3RegisterTest is V3Harness {
    ...
    function test_register_ecdsa_happyPath_writesBindingAndEmits() public {
        ...
        ZkqesVerifier.LeafInputs memory leaf   = _leafInputs(NULLIFIER);
        ZkqesVerifier.ChainInputs memory chain = _chainInputs(1);
        address pkAddr = ZkqesVerifier.toPkAddress(leaf.pkX, leaf.pkY);
        ...
        assertEq(uint8(status), uint8(ZkqesRegistryV3.Status.ACTIVE));
    ...
    function test_register_rsa_happyPath_routesThroughRsaSlots() public {
        ...
        ZkqesVerifier.LeafInputs memory leaf   = _leafInputs(NULLIFIER);
        ZkqesVerifier.ChainInputs memory chain = _chainInputs(0);
        address pkAddr = ZkqesVerifier.toPkAddress(leaf.pkX, leaf.pkY);
    ...
    function test_register_revertsOnFutureTimestamp() public {
        ...
        vm.expectRevert(ZkqesRegistryV3.BindingFromFuture.selector);
    ...
    function test_register_revertsOnTooOldBinding() public {
        ...
        vm.expectRevert(ZkqesRegistryV3.BindingTooOld.selector);
        registry.register(_zeroProof(), leaf, _zeroProof(), _chainInputs(1));
    }
    ...
    function test_register_revertsOnAlreadyBound() public {
        ...
        ZkqesVerifier.LeafInputs memory leaf2 = _leafInputs(bytes32(uint256(NULLIFIER) ^ 1));
        vm.expectRevert(ZkqesRegistryV3.AlreadyBound.selector);
        registry.register(_zeroProof(), leaf2, _zeroProof(), _chainInputs(1));
    }

    /// @dev declHash whitelist is enforced inside ZkqesVerifier.verify(); bad
