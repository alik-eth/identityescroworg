...
///         V2's \`QKBRegistry.escrow.t.sol\` semantically but uses the new
///         dual-proof entrypoints.
contract QKBRegistryV3EscrowTest is V3Harness {
    ...
    function test_registerEscrow_revertsOnPastExpiry() public {
        vm.expectRevert(QKBRegistryV3.EscrowExpiryInPast.selector);
    ...
    function test_registerEscrow_revertsOnDoubleRegister() public {
        uint64 expiry = uint64(block.timestamp + 365 days);
        _registerEscrow(ESCROW_ID, ARBITRATOR, expiry);
        vm.expectRevert(QKBRegistryV3.EscrowExists.selector);
        _registerEscrow(ESCROW_ID, ARBITRATOR, expiry);
    }
    ...
    function test_registerEscrow_revertsWhenBindingNotActive() public {
        ...
        QKBRegistryV3 r2 = new QKBRegistryV3(
    ...
    function test_revokeEscrow_revertsWhenNoEscrow() public {
        vm.expectRevert(QKBRegistryV3.NoEscrow.selector);
        _revokeEscrow(REASON);
    }
    ...
    function test_revokeEscrow_revertsOnDoubleRevoke() public {
        ...
        _registerEscrow(ESCROW_ID, ARBITRATOR, expiry);
        _revokeEscrow(REASON);
        vm.expectRevert(QKBRegistryV3.EscrowAlreadyRevoked.selector);
        _revokeEscrow(REASON);
    }
    ...
    function test_EscrowState_EnumDefault() public view {
        (bytes32 id, address arb, uint64 exp, uint64 pendingAt, QKBRegistryV3.EscrowState state)
            = registry.escrows(address(0xdead));
        assertEq(id, bytes32(0));
        assertEq(arb, address(0));
        assertEq(exp, 0);
        assertEq(pendingAt, 0);
        assertEq(uint8(state), uint8(QKBRegistryV3.EscrowState.NONE));
    }
    ...
    function test_registerEscrow_populatesReverseMapAndActiveState() public {
        ...
        (,,, uint64 pendingAt, QKBRegistryV3.EscrowState state) = registry.escrows(pk);
        assertEq(uint8(state), uint8(QKBRegistryV3.EscrowState.ACTIVE));
        assertEq(pendingAt, 0);
    }
    ...
    function test_revokeEscrow_setsStateRevoked() public {
        ...
        _registerEscrow(ESCROW_ID, ARBITRATOR, expiry);
        _revokeEscrow(REASON);
        (,,,, QKBRegistryV3.EscrowState state) = registry.escrows(pk);
        assertEq(uint8(state), uint8(QKBRegistryV3.EscrowState.REVOKED));
    }
    ...
    function test_NotifyReleasePending_OnlyArbitrator() public {
        ...
        vm.expectRevert(QKBRegistryV3.NotArbitrator.selector);
    ...
    function test_NotifyReleasePending_TransitionsActiveToPending() public {
        ...
        registry.notifyReleasePending(ESCROW_ID);
        (,,, uint64 pendingAt, QKBRegistryV3.EscrowState state) = registry.escrows(pk);
        assertEq(uint8(state), uint8(QKBRegistryV3.EscrowState.RELEASE_PENDING));
    ...
    function test_NotifyReleasePending_BlocksRevoke() public {
        ...
        vm.prank(ARBITRATOR);
        registry.notifyReleasePending(ESCROW_ID);
        vm.expectRevert(QKBRegistryV3.EscrowReleasePending.selector);
        _revokeEscrow(REASON);
    }

    function test_NotifyReleasePending_UnknownEscrowIdReverts() public {
        vm.expectRevert(QKBRegistryV3.UnknownEscrowId.selector);
        vm.prank(ARBITRATOR);
    ...
    function test_NotifyReleasePending_RevertsFromNonActiveState() public {
        ...
        vm.prank(ARBITRATOR);
        registry.notifyReleasePending(ESCROW_ID);
        vm.expectRevert(QKBRegistryV3.WrongState.selector);
        vm.prank(ARBITRATOR);
        registry.notifyReleasePending(ESCROW_ID);
    }
    ...
    function test_FinalizeRelease_TransitionsPendingToReleased() public {
        ...
        registry.finalizeRelease(ESCROW_ID);
        (,,,, QKBRegistryV3.EscrowState state) = registry.escrows(pk);
        assertEq(uint8(state), uint8(QKBRegistryV3.EscrowState.RELEASED));
    }
    ...
    function test_FinalizeRelease_BlocksRevoke() public {
        ...
        vm.prank(ARBITRATOR);
        registry.finalizeRelease(ESCROW_ID);
        vm.expectRevert(QKBRegistryV3.EscrowAlreadyReleased.selector);
        _revokeEscrow(REASON);
    }
