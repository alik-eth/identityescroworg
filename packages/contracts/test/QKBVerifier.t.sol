...
contract QKBVerifierTest is Test {
    ...
    function _baseInputs() internal pure returns (QKBVerifierV2.Inputs memory i) {
    ...
    function test_verify_packsInputsAndReturnsTrue() public {
        stub.setAccept(true);
        QKBVerifierV2.Inputs memory i = _baseInputs();
        assertTrue(QKBVerifierV2.verify(IGroth16VerifierV2(address(stub)), _zeroProof(), i));
    }

    function test_verify_returnsFalseWhenStubRejects() public {
        stub.setAccept(false);
        QKBVerifierV2.Inputs memory i = _baseInputs();
        assertFalse(QKBVerifierV2.verify(IGroth16VerifierV2(address(stub)), _zeroProof(), i));
    }
    ...
    function test_toPkAddress_matchesVmAddrAcrossPrivkeys() public view {
        ...
        // priv = 1 → G
        assertEq(QKBVerifierV2.toPkAddress(_splitToLimbsLE(GX), _splitToLimbsLE(GY)), vm.addr(1));
        ...
        assertEq(QKBVerifierV2.toPkAddress(_splitToLimbsLE(px2), _splitToLimbsLE(py2)), vm.addr(2));
        ...
        assertEq(QKBVerifierV2.toPkAddress(_splitToLimbsLE(px3), _splitToLimbsLE(py3)), vm.addr(3));
    }
}
