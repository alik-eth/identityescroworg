...
describe('ZkqesPresentationEcdsaLeaf — end-to-end (real Diia admin QES binding)', function () {
  ...
  // proof is covered in zkqes-ecdsa-chain.test.ts.
  this.timeout(60 * 60 * 1000);

  let circuit: CompiledCircuit;

  before(async () => {
    circuit = await compile('ZkqesPresentationEcdsaLeaf.circom');
  });
