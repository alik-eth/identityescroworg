...
#   ceremony/v5_3/qkb-v5_3-stub.zkey            (gitignored — *.zkey)
...
ZKEY0="$BUILD_DIR/qkb-v5_3-stub_0000.zkey"
ZKEY="$OUT_DIR/qkb-v5_3-stub.zkey"
...
if [[ ! -f "$ZKEY" ]]; then
  ...
  NODE_OPTIONS='--max-old-space-size=46080' \
    pnpm exec snarkjs zkey contribute "$ZKEY0" "$ZKEY" \
      --name="qkb-v5_3-stub-dev-1" -v -e="$ENTROPY"
fi
...
(
  cd "$PKG_DIR"
  sha256sum \
    "ceremony/v5_3/qkb-v5_3-stub.zkey" \
    "ceremony/v5_3/verification_key.json" \
    "ceremony/v5_3/Groth16VerifierV5_3Stub.sol" \
    "build/v5_3-stub/QKBPresentationV5.r1cs" \
