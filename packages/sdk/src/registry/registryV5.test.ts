...
describe('classifyV5RegistryRevert', () => {
  // V5.1: `NullifierUsed` selector dropped at contracts-eng `d3720c6`
  // when anti-Sybil moved to `usedCtx`. Coverage now lives in the
  // `AlreadyRegistered` and V5.1 (`CtxAlreadyUsed`) branches.
  it('maps AlreadyRegistered to registry.nullifierUsed', () => {
    const err = classifyV5RegistryRevert(REGISTRY_V5_ERROR_SELECTORS.AlreadyRegistered);
    expect(err?.code).toBe('registry.nullifierUsed');
  });

  it('maps CtxAlreadyUsed (V5.1) to registry.nullifierUsed', () => {
    const err = classifyV5RegistryRevert(REGISTRY_V5_ERROR_SELECTORS.CtxAlreadyUsed);
    expect(err?.code).toBe('registry.nullifierUsed');
  });
