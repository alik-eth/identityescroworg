...
A zero-knowledge proof of a qualified electronic signature. zkqes lets a holder of an EU-qualified electronic signature (eIDAS QES) authorize a wallet on chain without disclosing who they are.
...
2. Holder generates a canonical binding statement and signs it with their Diia QES (the QES private key never leaves the Diia app).
3. The browser builds a Groth16 witness over the V5 circuit (~3.9 M constraints) and proves — either in a Web Worker via snarkjs (~90 s on a flagship desktop browser, ~38 GB peak) or via the local `zkqes serve` native CLI prover from V5.4 (~14 s, ~3.7 GB peak) — that:
...
4. The wallet submits `register(...)` to `ZkqesRegistryV5_2` on Base. The contract verifies the Groth16 proof, calls the EIP-7212 P-256 precompile twice for the ECDSA chain, checks Merkle inclusion against the trusted-list + policy roots, and writes:
...
- **Specs** — [`docs/superpowers/specs/`](docs/superpowers/specs/)
  ...
  - [zkqes structural rename design](docs/superpowers/specs/2026-05-03-zkqes-rename-design.md)
- **Plans** — [`docs/superpowers/plans/`](docs/superpowers/plans/)
  ...
  - [zkqes rename orchestration](docs/superpowers/plans/2026-05-03-zkqes-rename-orchestration.md)
...
- [`packages/zkqes-cli`](packages/zkqes-cli) — `zkqes serve` localhost native prover (V5.4) bundled with rapidsnark; browser at `app.zkqes.org/ua/registerV5` auto-detects via `GET :9080/status` and offloads proof generation when present, with silent fallback to the in-browser worker.
